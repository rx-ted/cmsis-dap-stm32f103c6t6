...
#define DAP_PACKET_COUNT        8       ///< Buffers: 8 = Full-Speed (RAM limited).
...
#if defined ( BOARD_C6 )
#define GPIO_INIT(port, pinmask, mode, speed) \
  { GPIO_ConfigPin(port, (pinmask), (mode), (speed)); }
#else
...
// USART1 for CDC Port
// BOARD_C6 uses USART1 (PA9 TX / PA10 RX) for the CDC virtual serial port

#if defined ( BOARD_C6 )
#define USART_CDC_CLOCK(state)      RCC->APB2ENR |= RCC_APB2ENR_USART1EN
#define USART_CDC_GPIO_CLOCK(state) RCC->APB2ENR |= RCC_APB2ENR_IOPAEN
#define USART_CDC_REMAP()           /* no remap needed on C6 */
#else
...
#define USART_CDC_TX_PIN_MASK (1U << 9)
#define USART_CDC_RX_PIN_MASK (1U << 10)
#else
...
#define USART_CDC_BUFFER_SIZE (512)  /*  Size of Receive and Transmit buffers MUST BE 2^n */

// USB Connect Pull-Up

#if defined ( BOARD_C6 )

// USB Connect Pull-Up on C6 board: PA12 has a fixed 3.3V pull-up, no software
// control pin available. USB is always active when the board is powered.
...
#define DAP_PRODUCT         "CMSIS-DAP-C6"
#define DAP_SER_NUM         "0000"    // placeholder, overwritten by DAP_SER_NUM_INIT
#define DAP_SER_NUM_LEN     11        // "C6" + 8 hex digits + NUL
extern const char *GetUID_String(void);
#define DAP_SER_NUM_INIT(dst) strcpy((dst), GetUID_String())
...
#define PIN_nRESET_LOW()              \
  do {                      \
    /* output open-drain 50MHz, register mode = 0x07 */ \
    PIN_nRESET_PORT->CRH = (PIN_nRESET_PORT->CRH & ~PIN_MODE_MASK(PIN_nRESET_PIN - 8))  \
    | PIN_MODE(0x07, PIN_nRESET_PIN - 8); \
...
#define PIN_nRESET_HIGH()             \
  do {                      \
    ...
    PIN_nRESET_PORT->CRH = (PIN_nRESET_PORT->CRH & ~PIN_MODE_MASK(PIN_nRESET_PIN - 8))  \
    | PIN_MODE(0x08, PIN_nRESET_PIN - 8);    \
  } while (0)
#else
#define PIN_nRESET_LOW()              \
  do {                      \
